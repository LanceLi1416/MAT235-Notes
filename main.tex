...
\begin{enumerate}
    ...
    \begin{center}
        \begin{tikzpicture}
                ...
                \draw[thick,blue,->] (1,0) -- (3,0) node[below] {$\Delta f$, orthogonal to the level set};
...
In first year, we used the equation $y - f(a) = f'(a) (x-a)$ to estimate the function $y = f(x)$ at the point $a$. We want the error to decrease the fastest when we approach $x = a$. 
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw[thick,blue,domain=1.6:-1.6] plot (\x, {(\x)*(\x)}) node[below left] {$f(x)$};
            
            \draw[thick,red,domain=-1:2.2] plot (\x,{1.5*(\x)-0.5625}) node[below right] {$L(x)$};
            \draw[thick,DarkGreen,domain=-0.25:1.5] plot(\x,{3*\x-1.6875}) node[below left] {$g(x)$};
            
            \draw[thick,dashed] (0.75,0.5625) -- (0.75,0) node[below] {$a$};
        \end{tikzpicture}
    \end{center}
...
%TODO: end of reformatting section
...
We set $\Delta f(\vec{x}) = \vec{0}$. Solve $\vec{x}$, these are the \term{critical points}\index{Crtitial Point}.
...
\begin{itemize}
    ...
    \item The \term{Lagrange Multiplier Formula}\index{Lagrange Multiplier Formula} is $$\Delta f = \lambda \Delta g(+\mu \Delta h + \cdots)$$
\end{itemize}
