...
\begin{definition}[Parametric Equations]\index{Parametric Equations}
    Suppose that $x$ and $y$ are both given as functions of a third variable $t$, called a \term{parameter}, by the equations $$x = f(t) \qquad y = g(t)$$ which are called \term{parametric equations}. 
\end{definition}

Each value of $t$ determines a point $(x, y)$, which we can plot in a coordinate plane. As $t$ varies, the point $(x, y) = (f(t), g(t))$, varies and traces out a curve called a \term{parametric curve}\index{Parametric Curve}. 

\begin{example}
    Sketch and identify the curve defined by the parametric equations $$x = t^2 - 2t \qquad y = t + 1$$

    Each value of $t$ gives a point on the curve, as shown in the table. For instance, if $t = 1$, then $x = -1$, $y = 2$ and so the corresponding point is $(-1, 2)$. We plot the points $(x, y)$ determined by several values of the parameter and we join them to produce a curve. 
    ...
    \begin{minipage}[c]{0.45\linewidth}
        \begin{center}
            \begin{tikzpicture}
                ...
                \draw[thick,blue,domain=-0.5:0.75,smooth,->,>=stealth'] plot (\x, {(2+sqrt(\x*2+1))/2});
                \draw[thick,blue,domain=0.75:2.75,smooth,->,>=stealth'] plot (\x, {(2+sqrt(\x*2+1))/2});
                \draw[thick,blue,domain=2.75:4.2,smooth] plot (\x, {(2+sqrt(\x*2+1))/2});
                \draw[thick,blue,domain=-0.5:0.75,smooth,->,>=stealth'] plot (\x, {(2-sqrt(\x*2+1))/2});
                \draw[thick,blue,domain=0.75:2.75,smooth,->,>=stealth'] plot (\x, {(2-sqrt(\x*2+1))/2});
                \draw[thick,blue,domain=2.75:4.2,smooth] plot (\x, {(2-sqrt(\x*2+1))/2});
                
                \draw[red,fill=red] (4,-0.5) circle (.075cm) node[below,black] {$t=-2$};
                \draw[red,fill=red] (1.5,0) circle (.075cm) node[below,black] {$t=-1$};
                \draw[red,fill=red] (0,0.5) circle (.075cm) node[below left,black] {$t=0$};
                \draw[red,fill=red] (-0.5,1) circle (.075cm) node[left,black] {$t=1~$};
                \draw[red,fill=red] (0,1.5) circle (.075cm) node[above left,black] {$t=2$};
                \draw[red,fill=red] (1.5,2) circle (.075cm) node[above,black] {$t=3$};
                \draw[red,fill=red] (4,2.5) circle (.075cm) node[above,black] {$t=4$};
...
In the above example, we found a Cartesian equation in $x$ and $y$ whose graph coincided with the curve represented by parametric equations. This process is called \term{eliminating the parameter}. 

\begin{example}
    Sketch the curve represented by $$x = \cos{t} \qquad y = \sin{t} \qquad 0 \le t \le 2\pi$$

    If we plot points, it appears that the curve is a circle. We can confirm this by eliminating the parameter t. Observe that $$x^2 + y^2 + \cos^2{t} + \sin^2{t} = 1$$

    Because $x^2 + y^2 = 1$ is satisfied for all pairs of $x$ and $y$ values generated by the parametric equations, the point $(x, y)$ moves along the unit circle $x^2 + y^2 = 1$. Notice that in this example the parameter $t$ can be interpreted as the angle (in radians). As $t$ increases from $0$ to $2\pi$, the point ($x, y) = (\cos{t}, \sin{t})$ moves once around the circle in the counterclockwise direction starting from the point $(1, 0)$.
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw[thick,blue,->,>=stealth'] (1.414,-1.414) arc (-45:45:2cm);
            \draw[thick,blue,->,>=stealth'] (1.414,1.414) arc (45:135:2cm);
            \draw[thick,blue,->,>=stealth'] (-1.414,1.414) arc (135:225:2cm);
            \draw[thick,blue,->,>=stealth'] (-1.414,-1.414) arc (225:315:2cm);
            
            
            \draw[thick] (0,0) -- (1,1.73) node[right] {$~(\cos{t}, \sin{t})$};
            
            \draw[->,>=stealth',thick] (1,0) arc (0:60:1cm);
            \draw[red,fill=red] (1,1.73) circle (.075cm);
            
            \draw[red,fill=red] (2,0) circle (.075cm) node[above right,black] {$t=0$};
            \draw[red,fill=red] (0,2) circle (.075cm) node[above right,black] {$t=\frac{\pi}{2}$};
            \draw[red,fill=red] (-2,0) circle (.075cm) node[above left,black] {$t=\pi$};
            \draw[red,fill=red] (0,-2) circle (.075cm) node[below right,black] {$t=\frac{3\pi}{2}$};
            \draw[red,fill=red] (2,0) circle (.075cm) node[below left,black] {$t=2\pi$};
...
\begin{definition}[Cycloid]\index{Cycloid}
    The curve traced out by a point $P$ on the circumference of a circle as the circle rolls along a straight line is called a \term{cycloid}. If the circle has radius $r$ and rolls along the $x$-axis and if one position of $P$ is the origin, find parametric equations for the cycloid are $$x = r(\theta - \sin{\theta}) \qquad y = r(1 - \cos{\theta}) \qquad \theta \in \mathbb{R}$$
\end{definition}
...
\begin{center}
    \begin{tikzpicture}
        ...
        \draw[->,>=stealth'] (-9.4*\r,0) -- (18.6*\r,0);
            
        \draw[blue] (0,\r) circle (\r);
        \draw[blue] (2.2*\r,\r) circle (\r);
        \draw[blue] (5*\r,\r) circle (\r);
            
        \draw[thick,red,domain=-3*pi+\r:-pi-1.5 ,smooth,->,>=stealth'] plot ({(\x - sin(\x r))*\r},{(1 - cos(\x r))*\r});
        \draw[thick,red,domain=-pi-1.5 :pi-1.5  ,smooth,->,>=stealth'] plot ({(\x - sin(\x r))*\r},{(1 - cos(\x r))*\r});
        \draw[thick,red,domain=pi-1.5  :3*pi-1.5,smooth,->,>=stealth'] plot ({(\x - sin(\x r))*\r},{(1 - cos(\x r))*\r});
        \draw[thick,red,domain=3*pi-1.5:5*pi-1.5,smooth,->,>=stealth'] plot ({(\x - sin(\x r))*\r},{(1 - cos(\x r))*\r});
        \draw[thick,red,domain=5*pi-1.5:6*pi-2  ,smooth              ] plot ({(\x - sin(\x r))*\r},{(1 - cos(\x r))*\r});
            
        \draw[red,fill=red] (0                    ,0                  ) circle (0.075cm) node[black,below] {$P$};
        \draw[red,fill=red] ({\r*(2.2-sin(2.2 r))},{\r*(1-cos(2.2 r))}) circle (0.075cm) node[black,above] {$P$};
        \draw[red,fill=red] ({\r*(5  -sin(5   r))},{\r*(1-cos(5   r))}) circle (0.075cm) node[black,above right] {$P$};

        \def\offset{0.125};
        \draw[->,>=stealth'] ({-sqrt(2)*(\r+\offset)/2},{(sin(22.5)^2)*2*(\r+\offset)-\offset}) arc (225:135:{(\r+\offset)});
    \end{tikzpicture}
\end{center}

We choose as parameter the angle of rotation $\theta$ of the circle ($\theta = 0$ when $P$ is at the origin). Suppose the circle has rotated throug $\theta$ radians. Because the circle has been in contact with the line, we see that the distance it has rolled from the origin is $$| OT | = \mathrm{arc}~PT = r\theta$$
...
\begin{center}
    \begin{tikzpicture}
            ...
            \draw[thick,blue] (1.4,1) circle (1cm);
            
            \draw[thick,red,domain=-0.:3.7] plot ({\x - sin(\x r)},{1 - cos(\x r)});
            
            \draw (1.4,1) -- (1.4,0) node[below] {$T$};
            \draw[dashed] (1.4,1) -- (0.4,0.83) node[left] {$P$};
            \draw[dashed] (0.4,0.83) -- (0.4,-0.025) node[above left] {$x$};
            
            \draw[fill] (1.4,1) circle(0.025cm) node[right] {$C(r\theta, r)$};
            
            \node at (-0.25,-0.25) {$O$};
            \node at (0.6,0.4) {$y$};
            
            \draw (0,-0.75) -- (0,-1);
            \draw[->] (0.45,-0.875) -- (0,-0.875);
            \node at (0.7,-0.875) {$r\theta$};
            \draw[->] (0.95,-0.875) -- (1.4,-0.875);
            \draw (1.4,-0.75) -- (1.4,-1);
        \end{tikzpicture}
\end{center}

Therefore the center of the circle is $C(r\theta, r)$. Let the coordinates of $P$ be $(x, y)$. Then, 
$$x = | OT | - | PQ | = r\theta - r \sin{\theta} = r(\theta - \sin{\theta})$$
$$y = | TC | - | QC | = r       - r \cos{\theta} = r(1      - \cos{\theta})$$

Therefore parametric equations of the cycloid are $$x = r(\theta - \sin{\theta}) \qquad y = r(1 - \cos{\theta}) \qquad \theta \in \mathbb{R}$$ One arch of the cycloid comes from one rotation of the circle and so is described by $0 \le \theta \le 2\pi$. 

Although it is possible to eliminate the parameter $\theta$ from the equations above, the resulting Cartesian equation in $x$ and $y$ is very complicated \footnote{$x = r \arccos\left(1 - \frac{y}{r}\right)$, which only gives half of one arch. }, and not as convenient to work with as the parametric equations.

\section{Calculus with Parametric Curves}

\subsection{Tangents}

Suppose $f$ and $t$ are differentiable functions and we want to find the tangent line at a point on the parametric curve $x = f(t), y = g(t)$, where $y$ is also a differentiable function of $x$. Then the Chain Rule gives $$\frac{dy}{dt} = \frac{dy}{dx} \cdot \frac{dx}{dt}$$

If $\frac{dx}{dt} \neq 0$, we can solve for $\frac{dy}{dt}$: 
$$\frac{dy}{dx} = \frac{\frac{dy}{dt}}{\frac{dx}{dt}} \qquad \frac{dx}{dt} \neq 0$$

The curve has a horizontal tangent when $\frac{dy}{dt}$, provided that $\frac{dx}{dt} \neq 0$, and it has a vertical tangent when $\frac{dx}{dt} = 0$, provided that $\frac{dy}{dt} \neq 0$ (if both $\frac{dx}{dt} = 0$ and $\frac{dy}{dt} = 0$, then we would need to use other methods to determine the slope of the tangent). 

$\frac{d^2y}{dx^2}$ can also be found by $$\frac{d^2y}{dx^2} = \frac{d}{dx}\left( \frac{dy}{dx} \right) = \frac{\frac{d}{dt}\left(\frac{dy}{dx}\right)}{\frac{dx}{dt}}$$

\subsection{Areas}

We know that the area under a curve $y = F(x)$ from $a$ to $b$ is $A = \int_a^b F(x)\,dx$, where $F(x) \ge 0$. If the curve is traced out once by the parametric equations $x = f(t)$ and $y = g(t)$, $\alpha \le t \le \beta$, them we can calculate an area formula by using the Substitution Rule for Definite Integrals as follows: $$A = \int_a^b y\,dx = \int_\alpha^\beta g(t) f'(t)\,dt$$
...
$\Delta f = \begin{bmatrix} y^2e^z \\ 2xye^z \\ xy^2e^z \end{bmatrix}$

So $\Delta f(1,1,1) = \begin{bmatrix} e \\ 2e \\ e \end{bmatrix}$

The tangent plane:

$\begin{aligned}[t]
    \vec{n} \cdot \vec{r} &= \vec{n} \cdot \vec{r}_0 \\
    \begin{bmatrix} e \\ 2e \\ e \end{bmatrix} \cdot \vec{r} &= \begin{bmatrix} e \\ 2e \\ e \end{bmatrix} \cdot \begin{bmatrix} 1 \\ 1 \\ 1 \end{bmatrix} \\
    ex + 2ey + ez &= e + 2e + e \\
    &= 4e \\
    x + 2y + z &= 4
\end{aligned}$
...
\begin{enumerate}[label=\alph*)]
    \item Let $\vec{u} = (a,b)$ where $a^2 + b^2 = 0$. 
    ...
    $\begin{aligned}[t]
        D_{\vec{u}} f(\vec{a}) & = \lim_{h\to0} \frac{f(\vec{a} + h\vec{u}) - f(\vec{a})}{h}          \\
                               ...
                               & = \lim_{h\to0} \frac{1}{h} \frac{h^2ab}{h^2(a^2 + b^2)}              \\
                               & = \lim_{h\to0} \frac{ab}{h}                                          \\
                               & = \begin{cases}
                                        0          & y = 0 ~ (\vec{u} = (1,0))\\
                                        0          & x = 0 ~ (\vec{u} = (0,1))\\
    ...
    For continuity, $\begin{aligned}[t]
        \lim_{(x,y) \to (0,0)} f(x,y) &= f(0,0) = 0 \\
        x = 0: f &= \frac{0 \cdot y}{0^2 + y^2} = 0 \\
        y = 0: f &= \frac{x \cdot 0}{x^2 + 0^2} = 0 \\
        y = x: f &= \frac{x \cdot x}{x^2 + x^2} = \frac{1}{2} \neq 0
    \end{aligned}$ 
